node() {
        stage("POLL SCM") {
                git credentialsId: 'gitlab', url: 'https://github.com/kanapaima/CF_ECS_FARGATE_Jenkins.git/'
        }
                ...
                //Using Docker tool configured in Jenkins
               	docker.withTool('myDocker') {
                	sh "docker build -t awscli ."
                    	...
                    	sh "docker run -e \"AWS_ACCESS_KEY_ID=${AKIAI46BZJWIBRTEL4FA}\" -e \"AWS_SECRET_ACCESS_KEY=${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}\" --rm awscli aws cloudformation create-stack --stack-name jfrogexample1 --template-body file://New_ECS_Fargate_Para.yml --parameters file://parameters.json"
                        //It will show all EC2 instsnces in AWS account
                        sh "docker run -e \"AWS_ACCESS_KEY_ID=${AKIAI46BZJWIBRTEL4FA}\" -e \"AWS_SECRET_ACCESS_KEY=${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}\" --rm awscli aws ec2 describe-instances --output table"
                        sh "docker rmi awscli"
               }
            }
        }
